...
def issue_new_challan(
    ...
    # 1. Fetch the Officer's Badge Number
    role_info = (
    ...
    violation_key = challan_data.get("violation_type", "no helmet").lower().replace("_", " ")
    ...
    enforced_amount = 500
    for k, val in live_rates.items():
        k_clean = k.lower().replace("_", " ")
        if k_clean in violation_key or violation_key in k_clean:
            enforced_amount = val
            break

    # 2. Prepare full challan data including phone_number
    phone_number = challan_data.get("phone_number", "")
    new_challan = {
        ...
        "status": "unpaid",
        "issued_by": badge, 
        "location": challan_data.get("location", "Unknown"),
        "issued_at": datetime.utcnow().isoformat(),
        "phone_number": phone_number,  # 🔥 NEW: Store officer-provided phone
    }
    ...
    issued = response.data[0]

    # 4. 🔥 Fire WhatsApp notification in background (non-blocking)
    if phone_number:
        plate = issued.get("vehicle_number", "UNKNOWN")
        v_type = challan_data.get("violation_type", "Traffic Violation")
        msg = (
            f"[BTU ALERT] A traffic challan of ₹{enforced_amount} has been issued "
            f"for vehicle {plate}. Violation: {v_type}. "
            f"Please visit the citizen portal to pay."
        )
        from app.routers.citizen import _send_whatsapp_bg
        import threading
        threading.Thread(target=_send_whatsapp_bg, args=(phone_number, msg), daemon=True).start()
        print(f"📨 WhatsApp notice queued for {phone_number}")
